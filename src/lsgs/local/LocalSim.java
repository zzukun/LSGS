...
public class LocalSim {
		...
		for(int i=1;i<=1682;i++){
			for(int j=0;j<rates.size();j++){
				...
				if(r.getItemId() == i && r.getUserId() == userId_p){
					...
					//System.out.println(params.get(i-1).Sq);
					//System.out.println(Math.signum(r.getRating() - params.get(i-1).u_hat));
					//System.out.println(surprisal(r.getRating(),params.get(i-1).u_hat , params.get(i-1).b_hat));
	...
	private double local_sim_weight(int user_p, int user_q,double localSim) {
		int commonNum = getCommonNum(user_p, user_q);
		int smaller = commonNum;
		if(r<smaller)
			smaller = r;
	...
	/**
	 * the number of items which user p and user q rated in common
	 * @param user_p
	 * @param user_q
	 * @return
	 */
	private int getCommonNum(int user_p,int user_q) {
		LinkedList<Integer> list_p = new LinkedList<Integer>();
		LinkedList<Integer> list_q = new LinkedList<Integer>();
		...
		try {
			...
			while(sc.hasNext()){
				String line = sc.nextLine();
				String[] lineCut =  line.split("\\s");
				if(lineCut[0].equals(user_p + "")){
					list_p.add(Integer.valueOf(lineCut[1]));
				}else if (lineCut[0].equals(user_q + "")){
					list_q.add(Integer.valueOf(lineCut[1]));
				}
			}
			sc.close();
		} catch (Exception e) {
		...
		Collections.sort(list_q);
		int cnt = 0;
		
		for(int i=0;i<list_p.size();i++){
			for(int j=0;j<list_q.size();j++){
				if(list_p.get(i) == list_q.get(j)){
					cnt++;
				}
			}
		}
		System.out.println("The common number of " + user_p +" and " + user_q + " is : " + cnt);
		return cnt;
	}
